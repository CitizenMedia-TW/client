        ...
        <div className="flex flex-col w-[98%] justify-items-center my-4 gap-4">
          <div className="w-full grid justify-items-end">
            {notifications_Date.map((date, dateIndex) => (
              <>
                <div key={date.Date} className="w-full border-b-2 border-slate-200 text-gray-400">
                  {date.Date}
                </div>
                  ...
                  <div className="p-4">
                    {date.data.map((notification, index) => (
                      <>
                        <div
                          key={index}
                          className={`w-full flex flex-rol hover:bg-slate-100 ${notification.Read ? '' : 'border-l-8 border-slate-400 bg-slate-50'}`}
                        ...
                        >
                          ...
                          <div className="flex flex-col grow">
                            <div className="flex flex-row gap-2">
                              <div key={notification.Source} className="text-lg font-bold hover:text-blue-400">
                                {notification.Source}
                              </div>
                              <div key={notification.Event} className="text-md font-normal mt-0.5">
                                {notification.Event}
                              </div>
                            </div>
                            <div key={notification.Time} className="text-md font-normal text-slate-400 hover:text-slate-600">
                              {notification.Time}
                            </div>
                          </div>
