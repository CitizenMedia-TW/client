      ...
      <div className="w-8/12">
        <div className="w-full border-b-2 border-border">
          <section className="flex flex-row items-center">
            ...
            <p className="text-4xl font-bold flex m-3.5 text-primary">{session?.user?.name}</p>
          </section>
          <section className="flex flex-row gap-2 justify-end">
            ...
            <button className={library == 'btn' ? "font-bold text-slate-800 text-2xl -translate-y-1 hover:-translate-y-2" : "text-slate-500 hover:-translate-y-2"} onClick={() => toggle_lib()}>
              Library
            </button>
            <button className={stories == 'btn' ? "font-bold text-slate-800 text-2xl -translate-y-1 hover:-translate-y-2" : "text-slate-500 hover:-translate-y-2"} onClick={() => toggle_stories()}>
              Stories
            </button>
          </section>
        </div>
            ...
            {stories_blocks.map((story) => (
              <>
                ...
                <div className="my-4 flex flex-row w-full gap-4">
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="my-2 flex flex-row gap-3">
                      {story.tags.map((tag, tagIndex) => (
                        <div key={tagIndex} className="rounded-3xl bg-gray-200 hover:bg-gray-100 hover:-translate-y-1">
                  ...
                  <div className="w-6/12 h-36 bg-gray-300">
